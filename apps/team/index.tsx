...
export default function TeamApp({ paneId, onClose }: AppProps) {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            await corePost("/v3/user-chat/send", { recipient_id: showChat, content: sentMessage });
        } catch (e) {
    ...
    const handleInvite = async () => {
        ...
        try {
            ...
            if (res?.success) {
                ...
                if (res?.invite_code) toast.info(`Invite code: ${res.invite_code}`);
            }
        } catch (error: any) {
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    {!ctx.isOperational ? (
                        ...
                        <AnimatePresence mode="wait">
                            {/* Members Tab */}
                            {activeTab === "members" && (
                                <motion.div key="members" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-3">
                                        ...
                                        <div className="flex bg-black/20 p-1 rounded-xl border border-white/5">
                                            {(["all", "online", "offline"] as const).map(f => (
                                                <button
                                                    ...
                                                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${statusFilter === f ? "bg-emerald-500 text-black" : "text-white/50 hover:text-white"}`}
                                                >
                                    ...
                                    <div className="flex items-center gap-4 mb-4 text-xs text-white/40">
                                        <span className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-500" />{members.filter(m => m.status === "online").length} Online</span>
                                        <span className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-gray-500" />{members.filter(m => m.status === "offline").length} Offline</span>
                                    ...
                                    {isLoading ? (
                                        ...
                                        members
                                            .filter(m => {
                                                ...
                                                const matchesStatus = statusFilter === "all" || (statusFilter === "online" && m.status === "online") || (statusFilter === "offline" && m.status === "offline");
                                                ...
                                                <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/10 mt-2">
                                                    <div className="h-64 overflow-y-auto mb-3 space-y-3 pr-2 scrollbar-thin scrollbar-thumb-emerald-500/20 scrollbar-track-transparent">
                                                        {isLoadingChat ? (
                                                            ...
                                                            <div className="flex h-full items-center justify-center text-center text-xs text-emerald-500/30"><p>Schreib eine Nachricht<br />um den Chat zu starten</p></div>
                                                        ) : (
                                                            chatHistory.map(msg => {
                                                                const isMe = msg.sender_id !== showChat;
                                                                return (
                                                                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                                        <div className={`max-w-[85%] p-2.5 rounded-xl text-xs leading-relaxed ${isMe ? 'bg-emerald-500/20 text-emerald-50 rounded-tr-none' : 'bg-white/5 text-emerald-100/80 rounded-tl-none border border-white/5'}`}>
                                                                            {msg.content}
                                                                        </div>
                                                                    </div>
                                                                );
                                                    ...
                                                    <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                                                        <input
                                                            ...
                                                            autoFocus
                                                        />
                                                        <button onClick={handleSendMessage} disabled={!message.trim()} className="p-1.5 rounded-lg bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                                                            <Send size={14} />
                                                        </button>
                                                    </div>
                                                </div>
                                                ...
                                                return (
                                                    <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                                                        <div className="max-w-[85%]">
                                                            {!isMe && <div className="text-[10px] text-emerald-500/60 mb-1">{senderLabel}</div>}
                                                            <div className={`p-2.5 rounded-xl text-xs leading-relaxed ${isMe ? "bg-emerald-500/20 text-emerald-50 rounded-tr-none" : "bg-white/5 text-emerald-100/80 rounded-tl-none border border-white/5"}`}>
                                                                {msg.content}
                                                            </div>
                                                        </div>
                                                    </div>
                                    ...
                                    {activities.length === 0 ? (
                                        ...
                                        activities.map(activity => (
                                            <div key={activity.id} className="flex items-start gap-3 p-3 rounded-lg bg-white/[0.02] border border-white/5">
                                                <div className="p-2 rounded-lg bg-emerald-500/10"><Activity size={14} className="text-emerald-400" /></div>
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-1 text-[10px] text-emerald-500/40 mt-1">
                                                        <Clock size={10} />
                                                        {new Date(activity.timestamp).toLocaleString("de-DE", { hour: "2-digit", minute: "2-digit", day: "2-digit", month: "short" })}
                                                    </div>
                                                </div>
                                            </div>
                                        ))
                            ...
                            {/* Invite Tab */}
                            {activeTab === "invite" && (
                                <motion.div key="invite" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} className="space-y-4">
                                    <div className="text-center py-4">
                                        ...
                                        <p className="text-xs text-emerald-500/50 mt-1">Laden Sie Kollegen in Ihre Organisation ein</p>
                                    </div>

                                    <div className="space-y-3">
                                        <div>
                                            <label className="block text-[10px] text-emerald-500/60 uppercase tracking-wider mb-2">E-Mail-Adresse</label>
                                            <div className="relative">
                                        ...
                                        <div>
                                            <label className="block text-[10px] text-emerald-500/60 uppercase tracking-wider mb-2">Rolle</label>
                                            <div className="grid grid-cols-2 gap-2">
                                                {[
                                                    ...
                                                    <button
                                                        ...
                                                        className={`p-3 rounded-xl border text-left transition-all ${inviteRole === role.id ? "border-emerald-500/50 bg-emerald-500/10" : "border-white/10 hover:border-emerald-500/30"}`}
                                                    >
