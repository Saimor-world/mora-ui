...
export default function TerminalApp({ paneId }: AppProps) {
    ...
    const buildStatusOutput = useCallback(() => {
        ...
        const allPanes = usePaneStore.getState().panes;
        ...
        const statusLines = [
            ...
            `Aktive Fenster: ${allPanes.filter((p) => !p.minimized).length}`,
        ];
        if (sess?.expires_at) statusLines.push(`Läuft ab: ${sess.expires_at}`);
        if (sess?.history_count != null && sess?.history_limit != null) {
            statusLines.push(`Verlauf: ${sess.history_count}/${sess.history_limit}`);
        }
        if (sess?.close_reason) statusLines.push(`Schließ-Grund: ${sess.close_reason}`);
    ...
    const renderInputResult = useCallback((result: TerminalInputResult) => {
        ...
        if (result.stdout?.trim()) addLine("output", result.stdout.trimEnd());
        if (result.stderr?.trim()) addLine("error", result.stderr.trimEnd());
