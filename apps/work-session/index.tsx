...
import {
    AlertTriangle, ArrowUpRight, CheckCircle2, ChevronDown, ChevronUp, Clock3, Eye,
    FilePlus2, FolderOpen, Loader2, MoveRight, Pencil, PlayCircle, Search,
    SkipForward, Trash2, Type, Upload, XCircle,
} from 'lucide-react';
import { toast } from 'sonner';
import type { AppProps } from '@/lib/apps/types';
import { groupStepsBySegment } from './utils';
...
function isWriteKind(kind: string): boolean { return WRITE_KINDS.has(kind.toLowerCase()); }

const kindLabels: Record<string, string> = {
    create: 'Erstellen', update: 'Aktualisieren', move: 'Verschieben', rename: 'Umbenennen',
    intake: 'Einordnen', delete: 'Loeschen', search: 'Suchen', navigate: 'Navigieren',
    write: 'Schreiben', execute: 'Ausfuehren', patch: 'Aktualisieren',
};
...
const segmentOriginLabels: Record<string, string> = {
    planning: 'Ausgangsplan', continuation: 'Fortsetzung',
    navigation: 'Navigation', native: 'Direkte Aktion',
};

const stepStatusLabels: Record<WorkSessionStepStatus, string> = {
    pending: 'Geplant', running: 'Laeuft', done: 'Fertig',
    failed: 'Fehlgeschlagen', pending_confirmation: 'Wartet', skipped: 'Uebersprungen',
};
    ...
    return (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.06] p-3">
            <div className="flex items-start gap-2 mb-3">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                        <span className="text-[9px] uppercase tracking-wider text-amber-300/50">{step.action_label || kindLabels[step.kind.toLowerCase()] || step.kind}</span>
                    </div>
...
function TransitionGhostCard({ stepTitle, transitionType, message, segmentContext }: { stepTitle: string; transitionType: 'confirmed' | 'skipped' | string; message: string; segmentContext?: string; }) {
    ...
    return (
        <CommandReceipt tone={tone} icon={Icon} label={label}
            ...
            footer={footer} className="rounded-xl border-white/[0.06] bg-white/[0.02] shadow-none" />
    );
}

// ─── main app ────────────────────────────────────────────────────────────────

export default function WorkSessionApp({ paneId }: AppProps) {
    const { openPane } = usePaneStore();
    const paneData = usePaneStore((s) => s.getPane(paneId)?.data);
    const setActiveSession = useWorkSessionStore((state) => state.setActiveSession);

    const [plan, setPlan] = useState<WorkSessionPlan | null>((paneData?.plan as WorkSessionPlan | undefined) ?? null);
    const [isLoading, setIsLoading] = useState(!plan && !!paneData?.plan_id);
    const [error, setError] = useState<string | null>(null);
    const pollRef = useRef<ReturnType<typeof setInterval> | null>(null);

    useEffect(() => {
        const planId = paneData?.plan_id as string | undefined;
        if (plan || !planId) return;
        let cancelled = false;
        setIsLoading(true);
        coreGet(`/v3/work-session/plan/${encodeURIComponent(planId)}`, { isOptional: true })
            .then((data) => { if (!cancelled) { setPlan((data as WorkSessionPlan) ?? null); if (!data) setError('Plan nicht gefunden.'); } })
            .catch(() => { if (!cancelled) setError('Plan konnte nicht geladen werden.'); })
            .finally(() => { if (!cancelled) setIsLoading(false); });
        return () => { cancelled = true; };
    }, [paneData?.plan_id, plan]);
    ...
    useEffect(() => {
        ...
        dispatchWorkSessionPlan({
            ...
            state: plan.state, title: plan.title, summary: plan.summary, mode: plan.mode, scope: plan.scope, stats: plan.stats,
            ...
            {!isLoading && error && (
                <div className="flex-1 flex items-center justify-center p-8 text-center">
                    <CommandReceipt tone="red" icon={XCircle} label="Arbeitsplan nicht erreichbar" title={error}
                        body="Der Plan konnte gerade nicht geladen werden."
                        chips={[{ label: 'Keine Aktion ausgefuehrt' }, { label: 'Datenstand bleibt erhalten' }]}
            ...
            {!isLoading && !error && !plan && (
                <div className="flex-1 flex items-center justify-center p-8 text-center">
                    <CommandReceipt tone="slate" label="Arbeitsplan" title="Kein Plan geladen."
                        body="Mora zeigt hier nur einen vorhandenen Plan an."
                        chips={[{ label: 'Wartet auf Plan' }, { label: 'Universe bleibt aktiv' }]}
                        className="w-full max-w-xl" />
                </div>
            )}
                ...
                <div className="flex-1 overflow-y-auto">
                    <div className="px-5 pt-5 pb-4 border-b border-white/[0.05]">
                        <CommandReceipt tone={receiptTone} label={stateBadge?.label || 'Arbeitsplan'} title={plan.title}
                            body={plan.summary || 'Der Plan ist geladen und Mora zeigt hier die aktuelle Arbeit.'}
                            chips={[
