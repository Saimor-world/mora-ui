        ...
        const loadMembers = async () => {
            ...
            try {
                if (isAdmin) {
                    ...
                    setMembers(Array.isArray(membersRes) ? membersRes.map((u: any) => ({
                        id: u.id,
                        name: u.name || u.full_name || (u.email ? u.email.split('@')[0] : 'User'),
                        email: u.email,
                        role: u.role,
                    ...
                    setInvites(Array.isArray(invitesRes) ? invitesRes.map((invite: any) => ({
                        id: invite.id,
                                                    ...
                                                    {departments.map((dept) => (
                                                        <label key={dept.id} className="flex items-center gap-2 text-sm text-white/60">
                                                            <input
                                                                ...
                                                                onChange={(e) => setInviteDepartmentIds(prev =>
