...
import { coreGet, fetchAdminUsers, patchAdminUser, patchUserCompanyBinding, AdminUser, AdminUserPatch } from '@/lib/api/coreClient';
...
export default function UsersApp({ paneId }: AppProps) {
                ...
                if (isAdmin) {
                    ...
                    if (Array.isArray(membersRes)) {
                        setMembers(membersRes.map((u: any) => ({
                            id: u.id, name: u.name || u.full_name || (u.email ? u.email.split('@')[0] : 'User'),
                            email: u.email, role: u.role, status: 'active', lastActive: u.last_seen,
                        })));
                    } else { setMembers([]); }
                    if (Array.isArray(invitesRes)) {
                        setInvites(invitesRes.map((invite: any) => ({
                            id: invite.id, name: invite.email.split('@')[0], email: invite.email,
                            role: invite.role, status: 'invited',
                        })));
                    } else { setInvites([]); }
                }
            } catch {
    ...
    const handleInviteSubmit = async () => {
        ...
        const result = await createInvite({ email: inviteEmail, role: inviteRole as 'member' | 'manager' | 'admin', department_ids: inviteDepartmentIds });
        setIsSubmitting(false);
        if (result) {
            setInviteLink(result.invite_link ?? null);
            setInvites(prev => [...prev, { id: result.token ?? inviteEmail, name: inviteEmail.split('@')[0], email: inviteEmail, role: inviteRole, status: 'invited' as const }]);
        } else {
    ...
    const handleActiveToggle = async (memberId: string, currentlyActive: boolean) => {
        ...
        if (updated) {
            setMembers(prev => prev.map(m => m.id === memberId ? { ...m, status: updated.is_active ? 'active' : 'inactive' } : m));
            toast.success(updated.is_active ? 'User activated' : 'User deactivated');
        }
    };

    const closeInviteModal = () => { setShowInviteModal(false); setInviteEmail(''); setInviteDepartmentIds([]); setInviteLink(null); setLinkCopied(false); };

    const handleCopyInviteLink = () => {
        if (!inviteLink) return;
        navigator.clipboard.writeText(inviteLink).then(() => { setLinkCopied(true); setTimeout(() => setLinkCopied(false), 2000); });
    };

    const handleCompanyBindingChange = async (memberId: string, companyId: string) => {
        const updated = await patchUserCompanyBinding(memberId, companyId || null);
        if (!updated) return;
        setMembers(prev => prev.map(m => m.id === memberId ? { ...m, defaultCompanyId: updated.default_company_id || null } : m));
        toast.success(companyId ? 'Standard-Firma aktualisiert' : 'Standard-Firma entfernt');
    };
    ...
    return (
        <div className="h-full flex flex-col relative">
            ...
            <div className="p-4 border-b border-white/10">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-white/40 bg-white/10 px-2 py-0.5 rounded-full">{combinedMembers.length} members</span>
                    </div>
                    {isAdmin && (
                        <button data-testid="invite-button" onClick={() => setShowInviteModal(true)}
                    ...
                    <div className="flex bg-white/5 rounded-lg p-1">
                        {(['all', 'active', 'invited'] as const).map(f => (
                            <button key={f} onClick={() => setFilter(f)}
                                className={`px-3 py-1 text-xs rounded transition-all ${filter === f ? 'bg-emerald-500/20 text-emerald-400' : 'text-white/40 hover:text-white/60'}`}>
            ...
            <div className="flex-1 overflow-auto p-4">
                {isLoading ? (
                    <div className="flex items-center justify-center py-8"><RefreshCw className="text-emerald-400 animate-spin" size={24} /></div>
                ) : filteredMembers.length === 0 ? (
                    <div className="text-center py-8 text-white/40"><Users size={32} className="mx-auto mb-2 opacity-50" /><p>No members found</p></div>
                ) : (
                    <div className="space-y-2">
                        <AnimatePresence>
                            {filteredMembers.map((member, index) => {
                                ...
                                return (
                                    <motion.div key={`${member.status}:${member.id}`}
                                        ...
                                        <div className={`w-10 h-10 rounded-full ${roleConfig.bg} flex items-center justify-center`}>
                                            {member.avatar ? (
                                                ...
                                                <span className={`text-sm font-medium ${roleConfig.color}`}>{member.name.charAt(0).toUpperCase()}</span>
                                            )}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                <span className="text-sm font-medium text-white/90 truncate">{member.name}</span>
                                                {member.status === 'invited' && <span className="text-[10px] bg-yellow-500/20 text-yellow-400 px-1.5 py-0.5 rounded">Pending</span>}
                                            </div>
                                        ...
                                        {isAdmin && member.status !== 'invited' ? (
                                            <select value={member.role} onChange={(e) => handleRoleChange(member.id, e.target.value as AdminUserPatch['role'])}
                                        ...
                                        {isAdmin && member.status !== 'invited' && Array.isArray(member.companyOptions) && member.companyOptions.length > 0 && (
                                            <select value={member.defaultCompanyId || ''} onChange={(e) => handleCompanyBindingChange(member.id, e.target.value)}
                                                className="bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-xs text-white/70 focus:outline-none focus:ring-1 focus:ring-emerald-500/50 cursor-pointer"
                                                onClick={(e) => e.stopPropagation()} title="Standard-Firma">
                                                <option value="">No default company</option>
                                                {member.companyOptions.map(company => (<option key={company.id} value={company.id}>{company.name}</option>))}
                                            </select>
                                        )}
                                        {isAdmin && member.status !== 'invited' && (
                                            <button onClick={() => handleActiveToggle(member.id, member.status === 'active')}
                                        ...
                                        {member.lastActive && <div className="text-xs text-white/30 hidden group-hover:block">{member.lastActive}</div>}
                                    </motion.div>
                                );
                            })}
                        </AnimatePresence>
            ...
            <div className="p-3 border-t border-white/10 flex items-center justify-between">
                <div className="text-xs text-white/30">{viewMode === 'demo' ? 'Demo mode: limited integrations' : 'Invites are tenant-scoped'}</div>
                <button className="text-xs text-emerald-400 hover:text-emerald-300 flex items-center gap-1"><Settings size={12} />Roles & Permissions</button>
            </div>
                ...
                {showInviteModal && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                            onClick={(e) => e.stopPropagation()} className="bg-black/90 border border-white/20 rounded-2xl p-6 w-80">
                            {inviteLink ? (
                                <>
                                    <div className="flex items-center gap-2 mb-1"><Check size={18} className="text-emerald-400" /><h3 className="text-lg font-medium">Einladung gesendet</h3></div>
                                    <p className="text-xs text-white/40 mb-4">Teile diesen Link mit <span className="text-white/60">{inviteEmail}</span>. Er ist einmalig verwendbar.</p>
                                    <div data-testid="invite-link-display" className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-lg px-3 py-2 mb-4">
                                        <span className="flex-1 text-xs text-emerald-300 font-mono truncate">{inviteLink}</span>
                                        <button data-testid="copy-invite-link" onClick={handleCopyInviteLink}
                                            className="shrink-0 p-1 rounded hover:bg-white/10 text-white/50 hover:text-emerald-400 transition-all" title="Link kopieren">
                                            {linkCopied ? <Check size={14} className="text-emerald-400" /> : <Copy size={14} />}
                                        </button>
                                    </div>
                                    <button onClick={closeInviteModal} className="w-full py-2 rounded-lg bg-white/5 border border-white/10 text-white/60 hover:text-white/80 transition-all">Fertig</button>
                                </>
                            ) : (
                                <>
                                            ...
                                            <div className="flex gap-2">
                                                {(['member', 'manager'] as const).map(role => (
                                                    <button key={role} onClick={() => setInviteRole(role)}
                                                        className={`flex-1 py-2 rounded-lg text-sm transition-all ${inviteRole === role ? 'bg-emerald-500/20 border border-emerald-500/30 text-emerald-400' : 'bg-white/5 border border-white/10 text-white/60 hover:text-white/80'}`}>
                                            ...
                                            <div className="flex flex-col gap-1">
                                                ...
                                                {departments.map((dept) => (
                                                    <label key={dept.id} className="flex items-center gap-2 text-sm text-white/60">
                                                        <input type="checkbox" checked={inviteDepartmentIds.includes(dept.id)}
                                                            onChange={(e) => setInviteDepartmentIds((prev) => e.target.checked ? [...prev, dept.id] : prev.filter((id) => id !== dept.id))} />
                                                        {dept.name}
                                                    </label>
                                                ))}
                                            </div>
                                    ...
                                    <div className="flex gap-2 mt-6">
                                        <button onClick={closeInviteModal} className="flex-1 py-2 rounded-lg bg-white/5 border border-white/10 text-white/60 hover:text-white/80 transition-all">Cancel</button>
