...
/**
 * OwnerHome - Workspace selection for Owners/Admins/Managers
 * 
 * NOTE: The MoraOrb is now rendered by MoraOrbController (single source of truth)
 * This layer shows the workspace selection UI.
 */
...
export const OwnerHome: React.FC = () => {
    ...
    // Calculate visible companies BEFORE any early returns (Rules of Hooks)
    const visibleCompanies = useMemo(() => {
        if (role === 'owner' || role === 'admin' || role === 'manager') return companies;
        // Members/others see only non-demo workspaces
        return companies.filter((c) => !c.is_demo);
    }, [companies, role]);

    // AUTO-SKIP for Member/Demo: Load Café Aurora and go to core
    useEffect(() => {
        if (role === 'member' || role === 'demo') {
            // Find demo company (Café Aurora)
            const demoCompany = companies.find(c => c.is_demo);
            if (demoCompany) {
                setActiveCompany(demoCompany.id);
                loadDepartments(demoCompany.id);
                setViewLevel('core'); // Skip to core view
            }
        }
    }, [role, companies, setActiveCompany, loadDepartments, setViewLevel]);

    // Only show OwnerHome for Owner/Admin/Manager
    if (role === 'member' || role === 'demo') {
        return null; // Skip, handled by useEffect above
    }
    ...
    return (
        <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center gap-16 mt-32">
                {/* Title (Orb is rendered by MoraOrbController above) */}
