            ...
            <GlanceShell tone={tone}>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <DayLine
                        ...
                        empty={connectionLabel(feedState, 'Feed leer', 'RSS verbinden')}
                        onClick={nextFeed || feedState === 'configured' ? openFeed : feedState === 'loading' ? undefined : context.openIntegrations}
                    />
                    <DayLine
                        ...
                        {feeds.length > 0 ? (
                            <div className="flex flex-col gap-1.5">
                                <FeedThumbnailStrip
                                    items={feeds}
                                    limit={3}
                                    onClickItem={() => openFeed?.()}
                                    onClickMore={openFeed}
                                />
                                {feeds.slice(0, glanceRowLimit(context, 2, 3)).map((item) => (
                                    <FeedItemCard key={item.id} item={item} compact onOpen={openFeed} />
                                ))}
                                {feeds.length > glanceRowLimit(context, 2, 3) && (
                                    <AlleAnzeigenLink onClick={openFeed} extra={feeds.length - glanceRowLimit(context, 2, 3)} label="Feed öffnen" />
                                )}
                            </div>
                        ) : (
                            <ConnectionEmpty state={feedState} configuredLabel="Feed leer" connectLabel="RSS verbinden" />
                        )}
                    </div>
                    <div>
...
const DeinFeedWidget: React.FC<{ context: WidgetContext }> = React.memo(({ context }) => {
    ...
    const openFeed = context.openFeed ?? context.openIntegrations;
    const compact = context.compact;
    const tone: StatusTone = feeds.length > 0 ? 'info' : feedState === 'configured' ? 'ok' : 'neutral';
    ...
    return (
        <div className="flex h-full min-h-0 flex-col gap-2 overflow-hidden">
            ...
            {heroItem ? (
                ...
                <Empty>{feedState === 'configured' ? 'Feed leer — Quellen sind verbunden.' : 'Feed-Status wird geladen…'}</Empty>
            )}
        </div>
    );
});
DeinFeedWidget.displayName = 'DeinFeedWidget';
...
export const WIDGET_REGISTRY: Record<string, WidgetDefinition> = {
    ...
    deinFeed: {
        type: 'deinFeed', label: 'Dein Feed', hint: 'RSS-Signale mit Vorschaubildern', icon: <Radio size={14} />,
        defaultW: 3, defaultH: 4, minW: 2, minH: 3, surfaces: ['home', 'universe'],
        render: ({ context }) => <DeinFeedWidget context={context} />,
    },
    team: {
