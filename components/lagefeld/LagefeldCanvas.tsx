...
export function LagefeldCanvas({ state }: { state: FieldState }) {
  ...
  const bounds = useMemo(() => {
    const width = Math.max(
      720,
      ...state.cards.map((card) => card.x + CARD_W + 36),
    );
    const height = Math.max(
      CANVAS_MIN_H,
      ...state.cards.map((card) => card.y + CARD_H_ESTIMATE + 36),
    );
    return { width, height };
  }, [state.cards]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <svg
          aria-hidden="true"
          style={{ position: 'absolute', inset: 0, width: bounds.width, height: bounds.height, pointerEvents: 'none' }}
        >
          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
                ...
                <span aria-hidden="true" style={{ display: 'flex', gap: 4, fontSize: 10, opacity: 0.72 }}>
