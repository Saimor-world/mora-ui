...
Was kann ich für dich tun?`,
    ...
    const parseIntent = (text: string): { type: 'navigate' | 'search' | 'chat', target?: string } => {
        ...
        if (lower.includes('find') || lower.includes('such') || lower.includes('search')) {
            return { type: 'search', target: text.replace(/find|such|search|dokument|document/gi, '').trim() };
        }
    ...
    const executeSearch = (query: string) => {
        ...
        return `🔍 Ich öffne die Suche für **"${query}"**...`;
    };
            ...
            if (intent.type === 'navigate' && intent.target) {
                ...
                try {
                    const agentResponse = await moraAgentClient.chat({
                        ...
                        session_id: 'active_session', // Should use real session context
                ...
                <div className="p-4 border-t border-white/10">
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/30 focus:outline-none focus:border-emerald-500/50"
                        />
                        <button
                            onClick={sendMessage}
                    ...
                    <div className="flex gap-2 mt-2">
                        {['Zeig mir Operations', 'Was gibt es Neues?', 'Finde Projektplan'].map((suggestion) => (
                            <button
                                ...
                                className="text-xs px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full text-white/60 hover:text-white transition-colors"
                            >
                                {suggestion}
                            </button>
                        ))}
