...
/**
 * TerminalPane - MÔRA Command Line Interface
 * 
 * A terminal emulator for power users and MÔRA commands.
 * Supports both system commands and MÔRA AI commands.
 */
...
╠══════════════════════════════════════════════════════════════╣
║  AI COMMANDS:                                                ║
║    mora <frage>  - Frage an MÔRA stellen (via LLM)           ║
║    providers     - Verfügbare AI Provider anzeigen           ║
...
`;
    ...
    version: {
        ...
        handler: async () => "MÔRA OS v2.0.0-beta | Kernel: Organic Neural Core | Build: 2026.01.07"
    },
    whoami: {
    ...
    const [lines, setLines] = useState<TerminalLine[]>([
        {
            id: "welcome",
            type: "system",
            content: "MÔRA Terminal v2.0 - Tippe 'help' für verfügbare Befehle.",
            timestamp: new Date()
        }
    ]);
                ...
                if (!question) {
                    ...
                    try {
                        ...
                        if (response?.reply) {
                            ...
                            addLine("mora", `MÔRA${provider}: ${response.reply}`);
                        } else {
                            addLine("error", "MÔRA: Keine Antwort erhalten");
                        }
                    } catch (e: any) {
